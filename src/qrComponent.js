...
//
// Copyright (c) 2017 Craig OConnor
...
export default class QRcodeReact extends Component {
  static propTypes = {
  ...
  render() {
    ...
    let { className, margin, size, codeType, errorLevel, value, color, bgColor } = this.props;

    const qr = qrcode(codeType, errorLevel);
    qr.addData(value);
    qr.make();

    size   = size || 2;
    margin = (typeof margin === 'undefined') ? size * 4 : margin;

    let qrTable = [];
    let qrColumn;
    for (let r = 0; r < qr.getModuleCount(); r += 1) {
      qrColumn = [];
      for (let c = 0; c < qr.getModuleCount(); c += 1) {
        qrColumn.push(
          <td key={""+c} style={{
            ...
            width: size + "px",
            height: size + "px",
            backgroundColor: qr.isDark(r, c) ? color : bgColor,
          }}
          >
          </td>
        );
...
var qrcode = function(typeNumber, errorCorrectLevel) {
	...
	_this.createTableTag = function(cellSize, margin, color, bgColor) {
